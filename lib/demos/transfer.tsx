...
const TransferFlow = ({ monto, ctx }: { monto: number; ctx: DemoContext }) => {
  const [currentStep, setCurrentStep] = useState<
    'contacts' | 'accounts' | 'confirmation'
  >('contacts')
  const [selectedContact, setSelectedContact] = useState<
    (typeof mockContacts)[0] | null
  >(null)
  const [selectedAccount, setSelectedAccount] = useState<
    (typeof mockBankAccounts)[0] | null
  >(null)
    ...
    >
      ...
      <div className="">
        {mockContacts.map((contact) => (
          <motion.button
            key={contact.id}
            className="flex w-full items-center justify-between px-0 py-4 hover:bg-gray-50"
    ...
    >
      <motion.div className="flex flex-row px-0 py-4" layoutId="header">
        <Button
          ...
          className="-ml-2 mb-2 flex-shrink-0 p-2"
        >
      ...
      <div className="">
        <div className="p-0">
          <p className="mt-1 text-sm text-gray-600">{selectedContact?.name}</p>
        </div>
        {mockBankAccounts.map((account) => (
          <button
            key={account.id}
            className="flex w-full items-center justify-between px-0 py-4 transition-colors hover:bg-gray-50"
  ...
  const renderConfirmationStep = () => (
    ...
    >
      ...
      <div className="px-0 py-0">
        ...
        <motion.div className="py-8 text-center" layoutId="amount">
...
export const transferDemo: DemoDefinition = {
    ...
    if (qr.id === 'transfer') {
      ...
      setTimeout(() => {
        ctx.pushAssistantMessage(
          createMessage('assistant', [
            createTextBlock(
              'Perfecto, te ayudo con la transferencia. ¿A quién le quieres transferir y cuánto?',
            ),
          ]),
        )
      }, 1000)
      ...
      setTimeout(() => {
        ctx.pushAssistantMessage(
          createMessage('assistant', [
            createTextBlock(
              'Te ayudo con la recarga. ¿Cuál es tu número de teléfono?',
            ),
          ]),
        )
      }, 1000)
  ...
  onUserMessage: (text, ctx) => {
    ...
    if (
      text.toLowerCase().includes('transferir') &&
      text.toLowerCase().includes('mauro')
    ) {
      ...
      setTimeout(() => {
        ctx.pushAssistantMessage(
          createMessage('assistant', [
            createTextBlock(
              'Tenes tres contactos con ese nombre. ¿A cuál te referís?',
            ),
          ]),
        )
      ...
      setTimeout(() => {
        ctx.pushAssistantMessage(
          createMessage('assistant', [
            createTextBlock(
              'Te ayudo con la transferencia. ¿A quién le quieres transferir y cuánto?',
            ),
          ]),
        )
      }, 1000)
