    ...
    if (qr.label === 'Pagar servicios') {
      ...
      setTimeout(() => {
        ...
        ctx.pushAssistantMessage(
          createMessage('assistant', [
            createTextBlock(
              `Tenés ${services.length} servicios pendientes por un total de $${totalAmount.toLocaleString('es-AR')}${overdueCount > 0 ? `. Hay ${overdueCount > 1 ? '' : 'una '}factura${overdueCount > 1 ? 's' : ''} vencida${overdueCount > 1 ? 's' : ''}` : '.'}`,
            ),
        ...
        setTimeout(() => {
          ctx.pushAssistantMessage(
            createMessage('assistant', [
              createTextBlock(
                'Puedes decirme "pagar todas", "pagar Edesur" o seleccionar un servicio específico del listado.',
              ),
            ]),
          )
        }, 800)
    ...
    if (lowerText.includes('pagar todas') || lowerText.includes('pagar todo')) {
      setTimeout(() => {
        ctx.pushAssistantMessage(
          createMessage('assistant', [
            createTextBlock(
              'Perfecto, voy a procesar el pago de todos tus servicios pendientes.',
            ),
          ]),
        )
    ...
    if (flow) {
      ...
      if (isInformational) {
        ...
        setTimeout(() => {
          ...
          setTimeout(() => {
            ...
            const initialData = montoMatch
              ? { amount: parseFloat(montoMatch[1]) }
              : {}
                    ...
                    if (flow.id === 'transfer') {
                      ...
                      setTimeout(() => {
                        ctx.pushAssistantMessage(
                          createMessage('assistant', [
                            createTextBlock(
                              '¡Listo! La transferencia fue realizada',
                            ),
                            createWidgetBlock('confirmation', {
                              type: 'success',
                              title: '¡Transferencia realizada!',
                              recipient: (data.contact as { name: string })
                                .name,
                      ...
                      ctx.pushAssistantMessage(
                        createMessage('assistant', [
                          createTextBlock(
                            '¡Operación completada exitosamente! ¿Necesitas algo más?',
                          ),
                        ]),
                      )
                    }
      ...
      setTimeout(() => {
        ctx.pushAssistantMessage(
          createMessage('assistant', [
            createTextBlock(
              'No entendí bien qué necesitas. Puedo ayudarte a transferir dinero, ver tu saldo o pagar servicios.',
            ),
          ]),
        )
      }, 500)
